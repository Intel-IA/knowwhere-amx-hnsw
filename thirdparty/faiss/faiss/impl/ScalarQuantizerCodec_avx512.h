...
#include <faiss/impl/distances_amx.h>
...
struct DCTemplate_avx512<Quantizer, Similarity, 16> : SQDistanceComputer {
    ...
    void query_to_codes_batch_4_amx(
            const uint8_t* __restrict code_0,
            const uint8_t* __restrict code_1,
            const uint8_t* __restrict code_2,
            const uint8_t* __restrict code_3,
            ...
            float& dis3) {
        enable_amx();
        const uint8_t *mydata[4];
        float dis[4]={0};
        mydata[0] = (uint8_t*)code_0;
        mydata[1] = (uint8_t*)code_1;
        mydata[2] = (uint8_t*)code_2;
        mydata[3] = (uint8_t*)code_3;
        if(q_bf16==NULL){
          q_bf16=(uint16_t *)malloc(sizeof(uint16_t)*quant.d);
          for(int i=0;i<quant.d;i+=16){
            __m512 x = _mm512_loadu_ps((__m512i*)(q+i));
            __m256 y =(__m256) _mm512_cvtneps_pbh(x);
            _mm256_storeu_ps((float*)(q_bf16+i),y);
          }
        }
    ...
    void distances_batch_4(
            ...
            float& dis2,
            float& dis3) override {
          // if constexpr ((std::is_same_v<Quantizer, QuantizerBF16_avx512<16>>) && 
          //     (std::is_same_v<Similarity, SimilarityIP_avx512<16>>)) {
          //     query_to_codes_batch_4_amx(
    ...
    void distances_batch_amx(
            const idx_t* idxes,
            float * dis,
            int count) override {
            enable_amx();
            const uint8_t *mydata[16];
            for(int i=0;i<count;i++){
              mydata[i] = (uint8_t*)(codes + idxes[i] * code_size);
              prefetch_L2(mydata[i]);
            }
            if(q_bf16==NULL){
              q_bf16=(uint16_t *)malloc(sizeof(uint16_t)*quant.d);
              for(int i=0;i<quant.d;i+=16){
