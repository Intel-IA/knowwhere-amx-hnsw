...
void WithCosineNormDistanceComputer::distances_batch_amx(const idx_t* idxes,float *dis, int count){

    for(int i = 0; i < count; i++ ){
      prefetch_L2(inverse_l2_norms + idxes[i]);
      
    }
    basedis->distances_batch_amx(idxes, dis, count);

    for (int i = 0; i < count; i++) {
        dis[i] = dis[i] * inverse_l2_norms[idxes[i]] * inverse_query_norm;
    }
}
